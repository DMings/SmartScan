...
//    }
    ...
    private fun readResource(context: Context, id: Int): String {
...
//            }
//            while ({ n = inputStream.read(b);n }() != -1) {
//                out.append(String(b, 0, n))
//            }
     ...
     */
    fun loadProgram(context: Context, vertexShaderRes: Int, fragmentShaderRes: Int): Int {
        val vertexShaderId =
            compileVertexShader(readResource(context, vertexShaderRes))
        val fragmentShaderId =
            compileFragmentShader(readResource(context, fragmentShaderRes))
        return linkProgram(vertexShaderId, fragmentShaderId)
    }

    fun arrayToFloatBuffer(fArray: FloatArray): FloatBuffer {
        val fBuffer = ByteBuffer.allocateDirect(fArray.size * 4)
    ...
    fun arrayToShortBuffer(sArray: ShortArray): ShortBuffer {
        val sBuffer = ByteBuffer.allocateDirect(sArray.size * 2)
            .order(ByteOrder.nativeOrder())
            .asShortBuffer()
            .put(sArray)
        sBuffer.position(0)
